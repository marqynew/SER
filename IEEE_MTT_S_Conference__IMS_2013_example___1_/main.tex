...
After merging and preprocessing all four datasets, our final dataset contains 12,162 audio samples. The dataset is split into training (80\%) and test (20\%) sets using stratified sampling to maintain class balance across splits. Table \ref{tab:dataset} shows the distribution of samples across emotion categories and individual emotions.
...
\midrule
Negative & 7,692 & 63.2\% \\
\quad - Angry & 1,923 & 15.8\% \\
\quad - Disgust & 1,923 & 15.8\% \\
\quad - Fear & 1,923 & 15.8\% \\
\quad - Sad & 1,923 & 15.8\% \\
\midrule
Non-Negative & 4,470 & 36.8\% \\
\quad - Happy & 1,923 & 15.8\% \\
\quad - Neutral & 1,895 & 15.6\% \\
\quad - Surprise & 652 & 5.4\% \\
\midrule
Total & 12,162 & 100\% \\
\bottomrule
\end{tabular}
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.48\textwidth]{fig1_performance_comparison.pdf}
\caption{Performance comparison of three models on binary SER task. (a) Test accuracy showing CNN achieves 82.37\%, outperforming SVM (79.57\%) and Bi-LSTM (75.67\%). (b) F1-scores demonstrating balanced performance across both classes, with CNN achieving the highest score of 0.8658.}
...
% ============================================================
% FIGURE 2: Confusion Matrices (MOST IMPORTANT!)
% ============================================================
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{fig2_confusion_matrices.pdf}
\caption{Confusion matrices for three models on test set (2,432 samples). Rows represent true labels (top: Non-Negative, bottom: Negative), columns represent predictions. (a) SVM shows 17.8\% misclassification rate for Negative class. (b) CNN achieves lowest error rates (14.7\% and 16.8\%). (c) Bi-LSTM struggles with both classes (22.1\% and 23.4\% error rates).}
\label{fig:confusion}
\end{figure*}

\subsection{Confusion Matrix Analysis}

Analysis of confusion matrices (Figure \ref{fig:confusion}) reveals distinct error patterns for each model:
...
Figure \ref{fig:performance} provides a visual comparison of overall model performance, clearly showing CNN's superiority in both accuracy and F1-score metrics.
...
% ============================================================
% FIGURE 3: Training Curves (CNN vs Bi-LSTM)
% ============================================================
\begin{figure}[ht!]
\centering
\includegraphics[width=0.48\textwidth]{fig3_training_curves.pdf}
\caption{Training dynamics for deep learning models. (a-b) CNN shows stable convergence at epoch 25 with minimal train-validation gap. (c-d) Bi-LSTM exhibits fluctuating validation metrics and larger generalization gap, suggesting sensitivity to hyperparameters and potential underfitting.}
\label{fig:training}
\end{figure}
...
\textbf{Training Stability:} CNN training shows more stable convergence compared to Bi-LSTM, which exhibits larger fluctuations in validation metrics (see Figure \ref{fig:training}). This suggests CNNs are more robust to hyperparameter choices for this specific task. The training curves clearly demonstrate that CNN reaches optimal performance earlier and maintains consistent validation accuracy, while Bi-LSTM continues to fluctuate even in later epochs.
...
\begin{itemize}
\item \textbf{Ambiguous Emotions:} Emotions like "surprise" (classified as Non-Negative) are frequently misclassified as negative, likely due to high arousal levels shared with fear and anger. This ambiguity is reflected in the lower sample count for surprise (652 samples) compared to other emotions.

\item \textbf{Speaker Variability:} Misclassifications are more common for speakers with less expressive vocal characteristics or atypical prosody patterns. This is particularly evident across the four datasets (CREMA-D, RAVDESS, SAVEE, TESS) which feature different numbers of actors and recording conditions.

\item \textbf{Audio Quality:} Some samples from CREMA-D contain background noise or recording artifacts that impact feature extraction and subsequent classification. Dataset-specific preprocessing could potentially improve overall performance.

\item \textbf{Class Imbalance:} The dataset exhibits moderate class imbalance (63.2\% Negative vs 36.8\% Non-Negative), which may contribute to slightly higher recall on the majority class. However, the use of stratified splitting and balanced class weights mitigates this issue.
\end{itemize}
...
\item \textbf{Cross-Dataset Evaluation:} Testing generalization across the four datasets (CREMA-D, RAVDESS, SAVEE, TESS) individually could reveal dataset-specific biases and improve model robustness for real-world deployment.
\end{itemize}
